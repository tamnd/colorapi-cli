...
describes what users get. Pick a format with `-o`, or let colorapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
colorapi <command> -o table   # aligned columns for reading
colorapi <command> -o jsonl   # one JSON object per line, for piping
colorapi <command> -o json    # a single JSON array
colorapi <command> -o csv     # spreadsheet friendly
colorapi <command> -o tsv     # tab-separated
colorapi <command> -o url     # just the URL column
colorapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
colorapi <command> --fields id,title,url
```
...
```bash
colorapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
colorapi <command>            # a table, because this is a terminal
colorapi <command> | wc -l    # JSONL, because this is a pipe
```
